# -*- coding: utf-8 -*-
"""
Created on Mon Jun 19 13:45:05 2023
...
st.set_page_config(
    page_title="Pokemon Yellow",
    page_icon="🔅"
)

st.markdown("<h1 style='text-align: center; color: grey;'>Pokemon Yellow</h1>", unsafe_allow_html=True)
...
if pokemon != '':
    try :
        if bus.maps(pokemon, 'yellow') == 'not found':
            st.error("This pokemon can not be found in this generation")
        else:
            bus.maps(pokemon, 'yellow')
            do = True
            res_busqueda, minlevel, maxlevel, method, chance = bus.locations(pokemon, 'yellow')
            st.success("Found")
...
if do: 
    with l3:
        res_busqueda = bus.formatear(res_busqueda, 'yellow')
    ...
    if select != 'Select' and ok:
        ...
        formateado = bus.pinta_ruta(select, "yellow")
